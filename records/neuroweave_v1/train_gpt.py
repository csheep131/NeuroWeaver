...
class FineWebDataset:
    ...
    def get_batch(self, batch_tokens: int) -> tuple:
        ...
        if not self.shards:
            ...
            tokens = torch.randint(0, self.vocab_size, (num_sequences, self.seq_len + 1))
        else:
            # Load from shards
...
def compress_model(model: nn.Module, use_zstd: bool = False) -> tuple:
    ...
    for key, value in state_dict.items():
        if value.dtype == torch.float32:
            ...
            quantized[key] = (value / scale).round().clamp(-127, 127).byte()
            scales[key] = scale
        else:
            quantized[key] = value
    ...
    buffer = io.BytesIO()
    torch.save({"state_dict": quantized, "scales": scales}, buffer)
