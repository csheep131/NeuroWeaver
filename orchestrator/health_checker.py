    ...
    def get_early_warning_signs(self, run_id: str, metrics: Dict[str, Any]) -> List[str]:
        ...
        if len(loss_history) >= 20:
            ...
            if recent_oscillation > earlier_oscillation * 1.5:
                warnings.append(
                    f"⚠️ Loss-Oszillation nimmt zu: {earlier_oscillation:.4f} → {recent_oscillation:.4f}"
                )
        ...
        if len(gradient_history) >= 20:
            ...
            if earlier_avg > 0 and recent_avg > earlier_avg * 1.3:
                warnings.append(
                    f"⚠️ Gradient Norm steigt: {earlier_avg:.1f} → {recent_avg:.1f} (+{(recent_avg/earlier_avg-1)*100:.0f}%)"
                )
        ...
        if len(step_time_history) >= 20:
            ...
            if earlier_variance > 0 and recent_variance > earlier_variance * 2:
                warnings.append(
                    f"⚠️ Step-Time wird inkonsistent: Varianz {earlier_variance:.1f} → {recent_variance:.1f}"
                )
        ...
        if len(vram_history) >= 20:
            ...
            if denominator > 0:
                ...
                if slope > 10:  # >10 MB/Step
                    warnings.append(
                        f"⚠️ VRAM steigt kontinuierlich: +{slope:.1f} MB/Step"
                    )
