...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if _USE_FA3:
            ...
            # SDPA fallback: FA3 uses (B,T,H,D), SDPA uses (B,H,T,D)
            q_sdpa = q.transpose(1, 2)
            k_sdpa = k.transpose(1, 2)
            v_sdpa = v.transpose(1, 2)
            # SDPA doesn't handle GQA natively - expand KV heads
...
class GPT(nn.Module):
    def __init__(
        ...
        # MTP Heads Initialisierung
        self.mtp_heads = nn.ModuleList(
            [CastedLinear(model_dim, vocab_size, bias=False) for _ in range(mtp_num_heads)]
        )
        for head in self.mtp_heads:
            # WICHTIG: Zero-Init sorgt dafür, dass MTP das Training 
    ...
    def _init_weights(self) -> None:
        # 1. Token Embeddings initialisieren
        if self.tie_embeddings:
            nn.init.normal_(self.tok_emb.weight, mean=0.0, std=self.tied_embed_init_std)
        
        n = self.num_layers
        # Skalierungsfaktor für Projektions-Layer (Out-Proj und MLP-Down)
        proj_scale = 1.0 / math.sqrt(2 * n)
        
        # 2. Parameter-Banks initialisieren (Q, K, V, MLP)
        for i in range(n):
            # Attention-Gewichte
            nn.init.orthogonal_(self.qo_bank.data[i], gain=1.0)        # Q-Projektion
            nn.init.zeros_(self.qo_bank.data[n + i])                    # Out-Projektion (Zero Init)
            
            nn.init.orthogonal_(self.kv_bank.data[i], gain=1.0)        # K-Projektion
            nn.init.orthogonal_(self.kv_bank.data[n + i], gain=1.0)    # V-Projektion
            
            # MLP-Gewichte
            nn.init.orthogonal_(self.mlp_up_bank.data[i], gain=1.0)    # MLP Up (Hidden Expansion)
            nn.init.zeros_(self.mlp_down_bank.data[i])                  # MLP Down (Zero Init)
            
            # Skalierung der Projektions-Layer für stabilere Residual-Streams
            self.qo_bank.data[n + i].mul_(proj_scale)
            self.mlp_down_bank.data[i].mul_(proj_scale)

        # 3. Alle anderen Module (Heads, Bigram, VE) initialisieren
        for name, module in self.named_modules():
            if isinstance(module, nn.Linear):
                # Falls das Modul als _zero_init markiert wurde (wie unsere MTP-Heads)
                if getattr(module, "_zero_init", False):
                    nn.init.zeros_(module.weight)
                    if module.bias is not None:
                        nn.init.zeros_(module.bias)
                
                # Spezialfall für den Haupt-LM-Head (falls vorhanden und nicht tied)
                elif "lm_head" in name:
                    nn.init.normal_(module.weight, std=0.001) # Sehr kleine Init für BPB-Stabilität
                
                # Standard-Initialisierung für andere Matrizen (z.B. Bigram-Projektion)
                elif module.weight.ndim == 2 and module.weight.shape[0] >= 64 and module.weight.shape[1] >= 64:
                    nn.init.orthogonal_(module.weight, gain=1.0)
                else:
                    # Kleiner Fallback für sehr kleine Linear-Layer (Gates etc.)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, current_softcap: float | None = None) -> Tensor:
        n = self.num_layers
        ...
        # Decoder Layers mit Skip-Connections
        ...
        # Dynamisches Softcapping für den Haupt-Head
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                # k=0 -> n+2 Vorhersage, k=1 -> n+3 etc.
                offset = k + 1
                valid_t = seqlen - offset
                if valid_t <= 0:
                    continue
                
                # Hidden States und Targets entsprechend dem Offset verschieben
                mtp_hidden = x[:, :valid_t, :].reshape(-1, dim)
                mtp_targets = target_ids[:, offset :].reshape(-1)
                
                # MTP Logits Projektion
                mtp_logits_proj = mtp_head(mtp_hidden)
                
                # Auch für MTP das dynamische Softcapping anwenden
                mtp_logits = softcap * torch.tanh(mtp_logits_proj / softcap)
                
                mtp_loss_sum = mtp_loss_sum + F.cross_entropy(mtp_logits.float(), mtp_targets, reduction="mean")
                mtp_loss_count += 1
            
            if mtp_loss_count > 0:
                # Gewichteten MTP-Loss zum Main-Loss addieren
...
def main() -> None:
    ...
    if _USE_FA3:
        # H100: use Flash Attention 3 directly, disable SDPA backends
        enable_flash_sdp(True)
        enable_mem_efficient_sdp(False)
        enable_math_sdp(False)
    else:
        # RTX 3050 / non-H100: enable SDPA backends for fallback path
        enable_flash_sdp(True)
    ...
    while True:
        ...
        if step < (args.iterations - args.warmdown_iters):
            current_softcap = start_softcap
        else:
            wd_step = step - (args.iterations - args.warmdown_iters)
            wd_progress = min(wd_step / max(args.warmdown_iters, 1), 1.0)
            current_softcap = start_softcap + (end_softcap - start_softcap) * wd_progress
        ...
        for micro_step in range(grad_accum_steps):
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                # 2. Hier das current_softcap an das Modell übergeben
                loss = model(x, y, current_softcap=current_softcap)
